...
def Print(Score_html):  # print the result
    ...
    for i in result.keys():
        ...
        # print j
        print
        '%-10s%-2s%-10s%-8s%6s%8s%10s%6s%6s%5s%10s%-10s%-15s%s%s' % tuple(j)
        print
        " "
...
def main():
    ...
    try:
        ...
        print
        "User-name or password error, try again!"  # if name is not found, this is the reason
        main()
        exit()
    else:
        pass
    ...
    MyRequest = urllib2.Request('http://222.24.19.201/xscjcx.aspx?' + getdata, None,
