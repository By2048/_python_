...
formaction	URL	覆盖表单的 action 属性。（适用于 type="submit" 和 type="image-test"）
formenctype	见注释	覆盖表单的 enctype 属性。（适用于 type="submit" 和 type="image-test"）
formmethod	get post	覆盖表单的 method 属性。（适用于 type="submit" 和 type="image-test"）
formnovalidate	formnovalidate	覆盖表单的 novalidate 属性。如果使用该属性，则提交表单时不进行验证。
formtarget	_blank _self _parent _top framename	覆盖表单的 target 属性。（适用于 type="submit" 和 type="image-test"）
height	pixels %	定义 input 字段的高度。（适用于 type="image-test"）
...
type	button checkbox file hidden image-test password radio reset submit text	规定 input 元素的类型。
value	value	规定 input 元素的值。
width	pixels %	定义 input 字段的宽度。（适用于 type="image-test"）
'''
